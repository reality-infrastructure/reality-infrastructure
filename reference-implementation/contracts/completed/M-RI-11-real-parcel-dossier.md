...
=======================================================================
DONE REPORT — M-RI-11 (2026-07-27)
=======================================================================

1. PLANNED: Plan Gate (a)-(g) approved 2026-07-27 with amendments A1
   (alias table attested at Snapshot Gate), A2 (ltime tie-break:
   date, then source_id lexicographic, then record id), A3
   (conservative CCAO<->assessor linkage rationale). Snapshot Gate
   passed with rulings R1 (alias attestation; UNKNOWN not
   owner-designating; CSMA BLT LLC distinct from FIRSTKEY HOMES), R2
   (CRM record date = USER_date_disposed, ltime 25), R3 (55ju-2fs9
   dead endpoint; tax framing must not imply tax-clear), R4
   (observation traceability guardrails), R6 (two-entity
   current_owner frame per Plan Gate (e); ri_core _MAX_FRAME_SIZE=8
   surfaced, not improvised around). Observation mapping (O1-O4)
   approved with A2 inference gloss and A3 frozen hand-check values.

2. IMPLEMENTED: pilot/fetch_snapshots.py (run manually once,
   2026-07-27); pilot/snapshots/ (5 frozen snapshots, sha256s in
   MANIFEST); pilot/MANIFEST.md (retrieval entries, alias table,
   ltime mapping, linkage rationale, rulings); pilot/
   mass_assignments.md (frozen mass table); pilot/dolton_dossier.py
   (stdlib + ri_core public APIs, reads only snapshot bytes);
   tests/test_pilot.py (14 tests); tests/golden/pilot/
   dolton_dossier.out (13,548 bytes); .gitattributes extended to
   pilot golden path. CRM extract located (not authored) in
   the-registry-signal/data/nigel-shared/All_Inventory.geojson
   (sha256 8d42089d...7067), 1 of 740 features, copied verbatim.

3. TESTED: pytest -q -> 415 passed (401 prior + 14 new). Golden
   byte-match and cross-process determinism (PYTHONHASHSEED 1 vs
   99999) green. A3 hand-check masses asserted in-script and
   in-test: sslbda_disposition 0.40/0.40/0.10/0.10; current_owner
   0.48/0.32/0.12/0.08; counterfactual 0.80/0.20. The CRM-vs-deed
   contradiction REPRODUCES (inverted polarity vs the Plan Gate
   sketch: CRM asserts Sold 2017-01-01; SSLBDA appears in no queried
   public record 1999-2026).

4. COMMITTED: 5811b69 "M-RI-11: real-parcel Dolton title-belief
   dossier" (13 files, 2192 insertions). git diff shows zero ri_core
   changes.

5. PUSHED: origin/main (hash in git log; push follows this archive
