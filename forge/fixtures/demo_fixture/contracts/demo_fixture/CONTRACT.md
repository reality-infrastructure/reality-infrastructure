# CONTRACT: DEMO-FIXTURE — demo_fixture domain build (one line — fill in)
...
2. {{ordered steps; each phase ends with the full suite green before the next
   begins, one commit per phase, message prefixed `DEMO-FIXTURE-P<n>:`
...
- Single commit series on branch `contract/demo-fixture`; DONE report
